...
void ParallelEnumeration::printEmbedding(ui* embedding, ui size, ui idx){
    
    std::cout << "Embedding "<< idx << " : ";
...
void ParallelEnumeration::exploreGraphWithAutomorphismBreak(const Graph *data_graph, const Graph *query_graph, ui **candidates, ui *candidates_count, ui *order,
                                                       TreeNode *&tree, size_t* est_work_array, size_t thread_output_limit_num, size_t &call_count, 
            ...
            while (true){
                ...
                while (idx[cur_depth] < idx_count[cur_depth]){
                    ...
                    if (cur_depth == max_depth - 1)
                    {
                        process_embedding_count += 1;
                        visited_vertices[v] = false;
                        //printEmbedding(embedding, max_depth, process_embedding_count);
                        if (process_embedding_count >= thread_output_limit_num)
                        {
                            goto EXIT;
                        }
                    ...
                    else
                    {
                        call_count += 1;
                        cur_depth += 1;
                        //Breaking Automorphism in times of generating valid candidates
                        idx[cur_depth] = 0;
